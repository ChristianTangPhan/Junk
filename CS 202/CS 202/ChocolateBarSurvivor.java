...
public class ChocolateBarSurvivor
{
   ...
   public static void main(String[] args)
   {
      ...
      if (genderOfPerson == WOMAN)
      {
         // Calculate the BMR of a woman.
         basalMetabolicRate = WOMAN_BMR_CONSTANT + 
                             (WOMAN_WEIGHT_RATE * weightInPounds) + 
                             (WOMAN_HEIGHT_RATE * heightInInches) - 
                             (WOMAN_AGE_RATE * ageInYears);
      }
      else if (genderOfPerson == MAN)
      {
         // Calculate the BMR of a man.
         basalMetabolicRate = MAN_BMR_CONSTANT + 
                             (MAN_WEIGHT_RATE * weightInPounds) + 
                             (MAN_HEIGHT_RATE * heightInInches) - 
                             (MAN_AGE_RATE * ageInYears);
      }
      else 
      {
   ...
   public static char getGenderOfPerson()
   {
      // Declare a variable in which to store the user's value.
      char genderOfPerson; 
      ...
      // Get and store the value the user is providing for us.
      genderOfPerson = KEYBOARD.next().toUpperCase().charAt(0);
