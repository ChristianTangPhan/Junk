...
bool          laser_toggle      = false;  // laser state
bool          laser_pressed     = false;  // tracks if LASER button is held
...
void setup() 
{
  ...
  // Laser pin
  pinMode(IN5, OUTPUT);
  digitalWrite(IN5, LOW);  // <<<<<< laser OFF on startup
...
void loop() 
{
  ...
  {
    Serial.print("Direction received: 0x");
    Serial.println(last_direction, HEX);

    if      (last_direction == FORWARD) 
      forward();
    else if (last_direction == BACK)    
      back();
    ...
    // Handle laser toggle
    if (last_direction == LASER) 
    {
      if (!laser_pressed)  // only toggle once per press
      {
        laser_toggle = !laser_toggle;
        digitalWrite(IN5, laser_toggle ? HIGH : LOW);
        laser_pressed = true;
      }
    } 
    else 
    {
      laser_pressed = false;  // reset when LASER button released
    }
  }
}
